...
import { addPlayer, createGame, startGame } from '../engine/slaveEngine';
...
// A real 8-player game state survives a round trip and shrinks well below PeerJS's ~16 KB per-message ceiling.
let s = createGame('ABC123', 0);
for (let i = 0; i < 8; i++) s = addPlayer(s, `player-${i}-${'x'.repeat(10)}`, `Player ${i}`, 0);
s = startGame(s, 0);
