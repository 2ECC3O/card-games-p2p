...
function identity(name: string, watch = false): Identity {
  ...
  return { id: read('slave.id', 8), secret: read('slave.secret', 16), name, watch };
}
...
export default function App() {
  ...
  const [name, setName] = useState(() => localStorage.getItem('slave.name') ?? '');
  ...
  const [muted, setMuted] = useState(() => localStorage.getItem('slave.muted') === '1');
  ...
  const leaveToHome = useCallback((message: string | null) => {
    ...
    setGame(null);
    setNotice(message);
    sessionStorage.removeItem('slave.room');
    sessionStorage.removeItem('slave.watch');
  ...
  const enter = (n: TableNet) => {
    netRef.current = n;
    setNet(n);
    sessionStorage.setItem('slave.room', n.roomCode);
    if (n.me.watch) sessionStorage.setItem('slave.watch', '1');
  ...
  const validName = () => {
    ...
    localStorage.setItem('slave.name', trimmed);
    return trimmed;
  };
  ...
  const createRoom = async (e: FormEvent) => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      try {
        enter(await TableNet.host(roomCode, identity(player, isTournament(player)), createGame(roomCode, Date.now()), events));
        break;
      } catch (err) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('slave.name');
    if (!autoJoined.current && urlRoom && stored && sessionStorage.getItem('slave.room') === urlRoom) {
      void joinRoom(urlRoom, stored, sessionStorage.getItem('slave.watch') === '1');
    }
  ...
  if (!net || !game) {
    ...
    return (
      <main className="lobby" style={{ '--accent': '#5e3a6e', '--felt': '#3f2a4c' } as import('react').CSSProperties}>
        <nav className="room-nav" aria-label="Game navigation"><a href="../">← Card Games</a><span>Table 06 / Slave</span></nav>
        <div className="lobby-layout">
          <div className="flex flex-col gap-4">
            <header className="lobby-intro" data-mark="♚"><p className="edition">1–8 players · Against each other</p>
  ...
  const share = async () => {
    try {
      if (navigator.share) await navigator.share({ title: 'Slave P2P', text: `Join my Slave table ${game.roomCode}`, url });
      else {
  ...
  const toggleMute = () => {
    localStorage.setItem('slave.muted', muted ? '0' : '1');
    setMuted(!muted);
  };
  const activeName = game.players.find((p) => p.id === game.activeId)?.name ?? 'another player';

  return (
    <main className="table-room flex h-dvh flex-col overflow-hidden" style={{ '--accent': '#5e3a6e', '--felt': '#3f2a4c' } as import('react').CSSProperties}>
          ...
          {status === 'reconnecting' ? (
            ...
            game.round > 0 && (
              <>
                ...
                {me?.title && <span>You're <span className="text-slate-200">{me.title}</span></span>}
              </>
            )
          )}
        </div>
        ...
        {me && (game.phase === 'playing' || game.phase === 'exchange') && me.hand.length > 0 ? (
          ...
          <div className="mx-auto flex max-w-2xl gap-2 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:px-5">
            <button disabled={seated < 1} onClick={() => net.startGame()} className={`${button.primary} min-h-12 flex-1 text-base sm:min-h-14 sm:text-lg`}>
              {seated === 0 ? 'Waiting for players' : seated < MIN_TABLE ? `Start with ${MIN_TABLE - seated} bot${MIN_TABLE - seated === 1 ? '' : 's'}` : `Start game with ${seated} players`}
            </button>
          ...
          <p className="px-3 pb-3 text-center text-sm text-slate-400 sm:text-base" aria-live="polite">
            {game.phase === 'playing'
              ? me?.hand.length === 0 ? `You're out. Waiting for ${activeName}…` : `Waiting for ${activeName}…`
              : game.phase === 'settled'
