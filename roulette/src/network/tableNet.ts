...
import {
  addPlayer, applyAction, BET_MS, cleanName, hostTick, IDLE_MS, rejoinQueue, removePlayer, setConnected, startGame,
...
export class TableNet {
  ...
  private promote() {
    ...
    if (state.deadline) state = { ...state, deadline: now + BET_MS };
