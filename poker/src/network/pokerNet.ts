...
export class PokerNet {
  ...
  static async host(roomCode: string, me: Identity, game: GameState, events: NetEvents): Promise<PokerNet> {
    const net = new PokerNet(roomCode, me, events, await openPeer(PREFIX + roomCode));
    net.becomeHost(game, []);
    return net;
  }

  static async join(roomCode: string, me: Identity, events: NetEvents): Promise<PokerNet> {
    const net = new PokerNet(roomCode, me, events, await openPeer());
  ...
  private promote() {
    ...
    if (state.turnDeadline) state = { ...state, turnDeadline: now + TURN_MS };
